######################################################################################################################
#                      Code provided by AllenDowney provided: https://github.com/AllenDowney/ThinkDSP/blob/master/code/thinkdsp.py#L660
#
######################################################################################################################
...
class _SpectrumParent:
    ...
    def plot_power(self, high=None, log = False, **options):
        ...
        if self.full:
            fs, amps = self.render_full(high)
            if log:
                plt.loglog(fs, amps ** 2, **options)
            else:
                plt.plot(fs, amps ** 2, **options)
        else:
            i = None if high is None else find_index(high, self.fs)
            if log:
                 plt.loglog(self.fs[:i], self.power[:i], **options)
            else:
...
def add_pink_noise(signal:  np.array, att=np.log10(2)*10, freq_min = 500, freq_max = 1000, rate=44100) -> np.array:
...
def pink_spectrum(f, f_min=0, f_max=np.inf, att=np.log10(2)*10):
    """
...
if __name__ == '__main__':
    ...
    # mat = loadmat(r"D:\Uni work\Engineering Mathematics Work\Technical Project\Simulation_results\fft_dir\voltage.mat")
    # data = mat['data'][0]

    # rec = Wave(data, framerate = 25_000)
    # spectrum = rec.make_spectrum()
    # plt.plot(data)
    # plt.xlabel('Time (s)')
    # plt.ylabel('Electrode potential $(mV)$')
    ...
    # spectrum.plot_power(log=True)
    # plt.xlim([0, None])
    # plt.title('Power spectrum of TIDA active extracellular MEA recording', fontsize = 20)
    # plt.xlabel('Frequency (Hz)', fontsize = 20)
    ...
    # pink_noise = spectrum_noise(lambda x:pink_spectrum(x, f_max = 6000), samples = len(rec), rate= 25_000) 
    # pn_wav = Wave(pink_noise, framerate = 25_000)
    # pn_spectrum = pn_wav.make_spectrum()
    # pn_spectrum.plot_power(log=True)
    # plt.show()


    # pn = PinkNoise()
    # pn = pn.make_wave(duration = len(rec)/ 25_000, framerate = 25_000)
    # pn.plot()
    # plt.show()
    # pn_spectrum = pn.make_spectrum()
    # pn_spectrum.plot()
    # plt.show()

    # pn_spectrum.plot_power(log=True)
    # plt.title('Power spectrum of pink noise', fontsize = 20)
    # plt.xlabel('Frequency (Hz)', fontsize = 20)
    # plt.ylabel('Power', fontsize = 20)
    # plt.xticks(fontsize=20)
    # plt.yticks(fontsize=20)
    ...
    for index,data in enumerate(results["discriminatory score"]):
        plt.text(x=index -0.20 , y =data +0.01 , s=f"{str(data)[0:5]}" , fontdict=dict(fontsize=15))
    plt.xticks(rotation=20, fontsize = 15)
    plt.title("Wave_clus positive detection rate for different synthetic data scenarios",  fontsize = 20)
    plt.ylim([None, 0.3])
    plt.show()
